"""Load and render the guide-option support inventory."""
...
def render_guide_options(inventory: GuideOptionInventory | None = None) -> list[str]:
    """Render the deterministic guide-option section of the conformance document."""
    selected = inventory or load_guide_options()
    lines = [
        "## Guide options",
        "",
        *_wrap(
            "These guide choices are not fully visible from the check catalog. "
            "Each entry names the guide requirements it concerns; the requirement "
            "coverage section below links every identifier to the guide."
        ),
    ]
