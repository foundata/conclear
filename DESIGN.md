...
`release --resume <run-id>` resumes only after verifying the recorded source, configuration, tools, layouts, evidence and remote digests. It refuses to resume across a changed immutable input. A retry after a possibly completed candidate write generates a new candidate reference unless the old reference can be resolved conclusively to the expected digest and is still within its lifetime.
