...
- A **candidate reference** is the single-use Quay tag used to publish one accepted candidate before verification and promotion.
...
The configuration declares image definitions, Containerfile and context paths, the Quay destination, required platforms, native-testing requirements, runtime expectations, resource limits, test hooks, image-pin intent, candidate lifetime reductions and permitted exceptions. Paths resolve below the isolated source root and cannot escape through `..`, symlinks or archive entries. ConClear normalizes and compares the configured canonical source URL with the observed Git remote before using it in evidence.
...
| `doctor` | Validate configuration, rootless tools, supported versions, storage, target-platform execution, trust inputs, Quay access and public Sigstore transparency-service access without publishing content. |
...
Trivy is the default authoritative scanner for packages, vulnerabilities, secrets and configuration. It scans the build context for secrets, the Containerfile and image configuration for insecure settings, and the final layout for packages, vulnerabilities, secrets and configuration. A fixable `HIGH` or `CRITICAL` vulnerability rejects qualification unless an exact, approved and unexpired repository exception applies. A project may select the guide-permitted Syft and Grype combination for SBOM and vulnerability work, but Trivy still performs required secret and configuration scans and only one vulnerability result gates the release. Every rejecting scan runs against local content and the digest-addressed layout before publication.
...
Public foundata images are published to configured repositories on `quay.io`. Consumed images may come from another fully qualified authoritative registry. ConClear rejects short names and does not rewrite an upstream reference merely to prefer Quay.
...
- Buildah, Podman, Skopeo, scanner, Cosign and Quay adapters.
