...
Result data goes to standard output and this narration to standard error.
Every command narrates its phases and every external command it runs as INFO
...
`-q`/`--quiet` on the group drops that narration and never a warning or
error.
